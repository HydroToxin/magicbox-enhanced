...
class HarvestsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create harvest' do
    assert_difference('Harvest.count') do
      post harvests_url,
           params: { harvest: { dry_bud_weight: @harvest.dry_bud_weight, dry_trim_weight: @harvest.dry_trim_weight,
                                grow: @harvest.grow, havested_bud_weight: @harvest.havested_bud_weight, havested_trim_weight: @harvest.havested_trim_weight, havested_waste_weight: @harvest.havested_waste_weight } }
    end
