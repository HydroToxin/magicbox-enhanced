...
class GrowsController < ApplicationController
  ...
  def show
    ...
    @weeks_json = @grow.weeks.joins(:grow).where.not('grows.grow_status': %i[done
                                                                             aborted]).to_json(methods: %i[text url
                                                                                                           color])
    @observations_json = @grow.observations.all.to_json(methods: %i[start_date end_date text url])
