...
module Admin
  class SubjectsController < Admin::AdminController
    before_action :authenticate_user!

    before_action :set_grow, only: %i[index new show edit update destroy move_to]
    before_action :set_subject, only: %i[show edit update destroy move_to]
    ...
    def create
      ...
      respond_to do |format|
        if @subject.save
          format.html do
            redirect_to grow_subject_path(@subject.grow, @subject), notice: 'Subject was successfully created.'
          end
    ...
    def update
      respond_to do |format|
        if @subject.update(subject_params)
          format.html do
            redirect_to grow_subject_path(@subject.grow, @subject), notice: 'Subject was successfully updated.'
          end
    ...
    def subject_params
      params.require(:subject).permit(:name, :room_id, :grow_id, :birth_type, :mother_id, :strain_id)
    end
  end
end
